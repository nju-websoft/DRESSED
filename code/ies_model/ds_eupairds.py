...
from container.Evaluator import Evaluator
from container.State import State

class EUPairDataset_ds(object):
	def __init__(self, eu_pairs, env, dataConfig, deepset_style
				 , user=None  # if use pseudo user
				 , is_train=None # True/False
		...
		self.data = copy.deepcopy(eu_pairs) # deepcopy to avoid influence origin container by shuffle
		...
		self.doShuffle = doShuffle if doShuffle!=None else (True if is_train else False)
		#=======
		self.is_train = is_train
		self.byGreedy = not self.is_train
	...
	def _go_step_for_test(self, policy_model, state, forUReal=True):
		...
		if (state.done):  # skip over already-gold instances (by inserting an all-zero row)
			...
			step_input = self.env.getTransMatrix(state)
			state_vec_tuple = self.env.getStateVecTuple(state, self.deepset_style)  # ===== for deepset
	...
	def _get_episode_for_eu(self, eu, policy_model, useTensorboard = True):
		...
		#==== 1. init state
		timeStart=time.time()
		state0 = State(initType='origin', euPair=eu, dataConfig=self.dataConfig, user=self.user)
	...
	def get_eval(self, policy_model, batch_size=None, forDev=True, out_file=None):
		if batch_size==None:
			batch_start_idx=0
			batch_end_idx = self.size-1
		else:
