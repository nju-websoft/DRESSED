...
class PolicyNN_ds(object):
	def __init__(self, dataConfig
				 ...
				 , num_to_save=3
				 , restore_path=None
		, mlp_active=tf.nn.leaky_relu
		, mlp_hidden_nodes = [64]
		, encoder_active = tf.nn.relu
		, encoder_nodes = [300, 150]
				 ):
		print('init', name)
		self.sess = sess
		self.name = name
		self.restore_path = restore_path
		self.topk = dataConfig.topK

		self.max_cand_length = dataConfig.MAX_CAND_LENGTH
		self.num_features = dataConfig.NUM_FEATURES
		self.learning_rate = trainConfig.LEARNING_RATE

		self.mlp_active = mlp_active
		self.mlp_hidden_nodes = mlp_hidden_nodes
		assert len(self.mlp_hidden_nodes)>=1
		self.deepset_style = trainConfig.deepset_style
		self.encoder_active = encoder_active
		self.encoder_nodes = encoder_nodes
		assert len(self.encoder_nodes)>1
	...
	def _deepset_encoder(self, set_tensor, scope_name):
		...
		with tf.name_scope(scope_name):
			with tf.variable_scope(scope_name+'_v'): # for un-share variables defined by get_variable()
				set_input = set_tensor
				for node_idx in range(len(self.encoder_nodes)-1):
					set_h1 = tf.layers.dense(set_input, self.encoder_nodes[node_idx]#self.ENCODE_HIDDEN_UNITS#NUM_UNITS_FOR_SET_ENCODER  # hyper param
									, activation=self.encoder_active #TYPE_ACTIVATION_FOR_SET_ENCODER
									, name='set_h1' if node_idx==0 else 'set_h1_{}'.format(node_idx)
				...
				set_h2 = tf.layers.dense(set_h1, self.encoder_nodes[-1] # NUM_OUT_UNITS_FOR_SET_ENCODER  # hyper param
								, activation=self.encoder_active #TYPE_ACTIVATION_FOR_SET_ENCODER
								, name='set_h2'
								# settings for weights:  TODO: check the selection rules of initializer and regularizer
								, kernel_initializer=self.w_initializer,
	...
	def _encode_state(self):
		...
		concat_tuple = ()
		if 'curr' in self.deepset_style:
			self.state_curr_tensor = tf.placeholder(name='curr', shape=[None, self.topk.value-1, self.num_features], dtype=tf.float32)  # size = [triple_num * trans_curr_num, vec_dim]
		...
		if 'cand' in self.deepset_style:
			self.state_cand_tensor = tf.placeholder(name='cand', shape=[None, self.max_cand_length, self.num_features], dtype=tf.float32)  # size = [triple_num * trans_cand_num, vec_dim]
		...
		if 'disH' in self.deepset_style:
			self.state_disH_tensor = tf.placeholder(name='disH', shape=[None, self.max_cand_length, self.num_features], dtype=tf.float32)  # size = [triple_num * trans_disH_num, vec_dim]
		...
		self.state_vec_dim = self.encoder_nodes[-1] * item_num
	...
	def _encode_triple(self, tripleTensor, scope_name):
		...
		with tf.name_scope(scope_name):
			with tf.variable_scope(scope_name+'_v'): # for un-share variables defined by get_variable()
				input_tensor = tripleTensor
				for node_idx in range(len(self.encoder_nodes)-1):
					triple_h1 = tf.layers.dense(input_tensor, self.encoder_nodes[node_idx]
										 , activation=self.encoder_active
										 , name='h0' if node_idx==0 else 'h0_{}'.format(node_idx)
										 , kernel_initializer=self.w_initializer,
										 kernel_regularizer=tf.contrib.layers.l2_regularizer(0.01)
										 # settings for biases:
										 , bias_initializer=self.b_initializer  # tf.constant_initializer(0.0)
										 )
					input_tensor = triple_h1
				# if self.is_train:
				# 	triple_h1 = tf.layers.dropout(triple_h1, rate=self.dropout_rate)
				triple_h2 = tf.layers.dense(triple_h1, self.encoder_nodes[-1] # NUM_OUT_UNITS_FOR_SET_ENCODER  # hyper param
								, activation=self.encoder_active
								, name='set_h2'
								, kernel_initializer=self.w_initializer,
								kernel_regularizer=tf.contrib.layers.l2_regularizer(0.01)
	...
	def _build_model(self):
		...
		with tf.name_scope(self.name+'_input-layer'):
			self.triple_vec_list = tf.placeholder(name='candidates', shape=[None, self.num_features], dtype=tf.float32)  # size = [triple_num, 1] = [total_step_num * candidate_num_for_each_step, vec_dim]
			...
			self.state_disH_tensor = tf.placeholder(name='temp', shape=[None], dtype=tf.float32)  # for feeddict, will be inited according to self.deepset_style in self._encode_state
		...
		with tf.name_scope('encode-layer'):  # encode state
				...
				triple_encoded_list = self._encode_triple(self.triple_vec_list,'triple') \
					if 'triple' in self.deepset_style \
					else self.triple_vec_list
				# ==== encode state
				if self.deepset_style=='x':
					print('==========DeepSets is removed!, deepset_style=', self.deepset_style, )
		...
		with tf.name_scope("mlp-layer"): # encode triple+state
			h_input = self.input_vec
			for node_idx in range(len(self.mlp_hidden_nodes)):
				h1 = tf.layers.dense(h_input, self.mlp_hidden_nodes[node_idx]
									 , activation=self.mlp_active
									 , name = 'h1' if node_idx==0 else 'h1_{}'.format(node_idx)
			...
			self.h_out = tf.layers.dense(h1, self.NUM_OUT_UNITS, activation=self.mlp_active
			                             , name='h_out'
			                             , kernel_initializer=self.w_initializer,
			                             kernel_regularizer=tf.contrib.layers.l2_regularizer(0.01)
			                             , bias_initializer=self.b_initializer  # tf.constant_initializer(0.0)
	...
	def _predict_act_probs(self, step_triple_vec_list, step_cand_num, step_state_list_tuple=([],[],[])):
		...
		act_probs = self.sess.run(self.softmax_out
			...
			# 		if self.is_train else self.trainConfig.DROPOUT_RATE_FOR_TRAIN
			# , self.dropout_rate: self.dropout_rate_for_train
			# 		if self.is_train else 0.0
											   })
