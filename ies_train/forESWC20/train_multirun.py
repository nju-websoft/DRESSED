...
import time
from ies_train.train_for_eval import prepareParams, _initSummBatch, sutl, tf, np

RESULT_LIST_IDX = 6

def _train_one_model(repeat_idx, fold_idx, model, trainConfig, datasetTuple, summwTuple, batchSizeTuple, batchNumTuple, logger):

	split_num = len(datasetTuple)
	dev_ds_idx = 1 if split_num>2 else None  # train/dev/test or train/test
	do_test = True
		...
		for epoch_idx in range(trainConfig.EPOCH_NUM): # epoch
			...
			for ds_idx, dataset in enumerate(datasetTuple):  # train/dev/test
				...
				if (epoch_idx+1) % trainConfig.DEV_STEP != 0 and ds_idx > 0 \
					...
					continue
				if split_num>2 and ds_idx>1 and not do_test: # only run test(ds_idx>1) when dev is not bad when has dev split(split_num>2)
					continue
					...
					if dataset.has_next():
						...
						if ds_idx == 0:  # === train
							...
							if batch_data is None: # jump over illegal eu, already gold at init
								...
								continue
							if model.is_try:
								loss,neg,a,b,c,d,e,f,g = model.update(batch_data,useTensorboard=useTensorboard)
								loss,_,merged_summ = model.update(batch_data, useTensorboard=True)
								print('loss:',loss)
								print('neg:',neg)
								print('r0:',len(a),list(a))
								print('nr:',len(b),list(b))
								print('p0:',len(c),list(c))
								print('cp:',len(d),list(d))
								print('lp:',len(e),list(e))
								print('rl:',len(f),list(f))
								print('na:',len(g),list(g))
							else:
