...
'''
from data.DataConfig import *
from data.TrainConfig import *
...
def prepareParams(argsList=None
	...
	model_save_path_full = default_LOG_DIR + opt.rlName + '_' + opt.dataSplit + '_' + str(
		opt.dsName.name) + '_' + opt.run_name + '_' + str(signature)
	#========= logger
	...
	dataConfig = DataConfig(
		...
		, foldId = opt.foldId
		, ftypes = opt.ftypes
		, initSumm = opt.initSumm
		, encodeStyle = opt.encodeStyle
		, cleaned = opt.cleaned
		, byUser = opt.byUser
		, edistinct = opt.edistinct
		, forUreal = forUreal
		, gamma=opt.gamma
		, gamma_out=opt.gamma_out
		, useTotalReward=opt.useTotalReward
		, is_try=opt.is_try
		, reward_strategy=opt.reward_strategy
	)
	trainConfig = TrainConfig(
		num_repeats = opt.num_repeats
		# , num_features #=== to be set latter
		# , is_train #=== will be dynamically set according to data
		, do_debug=opt.do_debug
		...
		, type_activation = opt.type_activation
		, type_activation_out=opt.type_activation_out
		, num_h1_units = opt.num_h1_units
		, dropout_rate_for_train = opt.dropout_rate
		, name = opt.run_name
		, signature = signature
		#====== for deepset
		, deepset_style = opt.deepsetStyle
		, deepset_encode_hidden_units = opt.deepsetHidden
		, deepset_encode_out_units = opt.deepsetOut
		, use_neg=opt.use_neg

	)
	env = Env(dataConfig=dataConfig)
	trainConfig.setNumFeatures(env.getFDim())
	trainConfig.setModelFileName(model_save_path_full)
	# print('trainConfig:',trainConfig.USE_NEG, opt.use_neg)

	umodel = UserPolicy('learned', dataConfig=dataConfig)  # 'z'
	...
	useAllLabels = trainConfig.USE_ALL_LABELS
	if (dataConfig.dataSplit =='e5fold'):
		if dataConfig.foldId is not None and dataConfig.foldId >= 0:  # ==== only one fold
			print('dasplit:', dataConfig.dataSplit, ', has foldId=', dataConfig.foldId)
			train, dev, test = dataConfig.getDataE5Fold_TrainDevTest(dataConfig.foldId)  #=====
			ds_train = dataClass(train, env, dataConfig, deepsetStyle=trainConfig.deepsetStyle,
			                     name='trainf' + str(dataConfig.foldId), user=umodel, isTraining=True,
			                     doShuffle=True, useAllLabels=useAllLabels)
			ds_dev = dataClass(dev, env, dataConfig, deepsetStyle=trainConfig.deepsetStyle,
								name='devf' + str(dataConfig.foldId),
								user=umodel, isTraining=False,
								doShuffle=False, useAllLabels=useAllLabels)
			ds_test = dataClass(test, env, dataConfig, deepsetStyle=trainConfig.deepsetStyle,
			                    name='testf' + str(dataConfig.foldId), user=umodel, isTraining=False,
			                    doShuffle=False, useAllLabels=useAllLabels)
			datasetList = [(ds_train, ds_dev, ds_test)]
		else:  # === all 5 fold
			print('dasplit:', dataConfig.dataSplit, ', no foldId=', dataConfig.foldId)
			trainList, devList, testList = dataConfig.getDataE5Fold_TrainDevTest() #=====
			datasetList = []
			for fold_idx in range(5):
				ds_train = dataClass(trainList[fold_idx], env, dataConfig, deepsetStyle=trainConfig.deepsetStyle,
				                     name='trainf' + str(fold_idx),
				                     user=umodel, isTraining=True,
				                     doShuffle=True, useAllLabels=useAllLabels)
				ds_dev = dataClass(devList[fold_idx], env, dataConfig, deepsetStyle=trainConfig.deepsetStyle,
								name='devf' + str(fold_idx),
								user=umodel, isTraining=False,
								doShuffle=False, useAllLabels=useAllLabels)
				ds_test = dataClass(testList[fold_idx], env, dataConfig, deepsetStyle=trainConfig.deepsetStyle,
	...
	print('env:',str(env))
	logger.info('userModel:'+str(puser._getModelFilePath(umodel.modelDir, forModel=True)))

	dataConfig.loadTables()  #====== time consuming
...
def _initSummBatch(trainConfig, datasetList, repeat_idx, withSumm=True):
	...
	for fold_idx, datasetTuple in enumerate(datasetList):  # for folds
		...
		for ds_idx, dataset in enumerate(datasetTuple):  # train/dev/test
			...
			batch_size = int(np.min([dataset.size, trainConfig.BATCH_SIZE])) # all use batch (important!) # removed: if ds_idx == 0 else dataset.size  # only split to batches for train data(i.e. ds_idx==0)
			batchSizeTuple.append(batch_size)

			max_batch_num = int(np.round(dataset.size / float(batch_size)))
			batch_num = int(np.min([max_batch_num, trainConfig.BATCH_NUM])) # all use batch (important!) # removed: if ds_idx == 0 else 1  # only split to batches for train data(i.e. ds_idx==0)
